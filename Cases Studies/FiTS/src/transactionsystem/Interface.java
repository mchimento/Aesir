...
public class Interface {
	...
	public void USER_depositFromExternal(Integer uid, Integer sid, String to_account_number, int amount)
	{
		UserInfo u = ts.getUserInfo(uid);
		UserSession s = u.getSession(sid);
	...
	public boolean USER_payToExternal(Integer uid, Integer sid, String from_account_number, int amount)
	{
		UserInfo u = ts.getUserInfo(uid);
		UserSession s = u.getSession(sid);
		...
		if (u.getAccount(from_account_number).getBalance() >= amount) {
			...
			u.withdrawFrom(from_account_number, (int)total_amount);
			return true;
		}
	...
	public boolean USER_transferToOtherAccount(Integer from_uid, Integer sid, String from_account_number, Integer to_uid, String to_account_number, int amount)
	{
		...
		if (from_u.getAccount(from_account_number).getBalance() >= total_amount) {
			from_u.withdrawFrom(from_account_number, (int)total_amount);
	...
	public boolean USER_transferOwnAccounts(Integer uid, Integer sid, String from_account_number, String to_account_number, int amount)
	{
		UserInfo u = ts.getUserInfo(uid);
		UserSession s = u.getSession(sid);
		UserAccount 
