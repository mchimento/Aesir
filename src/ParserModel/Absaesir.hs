...
module ParserModel.Absaesir where
...
data TriggerList = CECollection [CEElement] WhereClause
  deriving (Eq, Ord, Show, Read)

data CEElement = CEct CompoundTrigger | CEid Id | CEidpar Id
  deriving (Eq, Ord, Show, Read)

data TriggerVariation = EVENil | EVEntry | EVExit [Vars]
  deriving (Eq, Ord, Show, Read)
...
data PropKind = PropKindNormal States Transitions
  deriving (Eq, Ord, Show, Read)
...
data Transition
    = Transition NameState NameState JML Id JML [Action]
  deriving (Eq, Ord, Show, Read)

data Action = Act CondExp
  deriving (Eq, Ord, Show, Read)
